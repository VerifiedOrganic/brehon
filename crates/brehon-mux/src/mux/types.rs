...
/// Keep this comfortably below a full-frame terminal redraw budget so large
/// worker output bursts are amortized instead of freezing operator input.
pub(super) const MAX_OUTPUT_BYTES_PER_POLL: usize = 64 * 1024;
...
/// Maximum delayed prompts dispatched from one state-machine tick.
pub(super) const MAX_READY_PROMPT_DISPATCHES_PER_TICK: usize = 1;
