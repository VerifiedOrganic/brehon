...
impl Mux {
    ...
    fn tick_pane_state_machine_impl(&mut self, rt: &tokio::runtime::Handle, now: Instant) {
        ...
        for pane_id in pane_ids {
            ...
            if let Some(pane) = self.panes.get_mut(&pane_id) {
                ...
                {
                    if now.saturating_duration_since(last_restart) >= Duration::from_secs(60) {
            ...
            // 3. Wedged pane detection & recovery trigger (moved before state_change so we recover before busy-to-ready fast path triggers)
            let mut should_recover = false;
            let mut recover_reason = String::new();
            ...
            if let Some(pane) = self.panes.get(&pane_id) {
                if pane.is_agy_or_opencode_supervisor() {
                    ...
                    let is_dead = matches!(
                        pane.pane_state(),
                        Some(PaneState::Dead {
                            reason: crate::pane::DeathReason::Quarantined(_),
                            ..
                        })
                    ) == false
                        && (pane.exited
                    ...
                    if is_dead {
                        should_recover = true;
                        recover_reason = "process_exited".to_string();
                        error_msg = "Process exited or entered terminal Dead state".to_string();
                    } else if let Some(PaneState::Busy { delivered_at, .. }) = pane.pane_state() {
                        let duration_busy = now.saturating_duration_since(*delivered_at);
                        if duration_busy >= super::MAX_TURN_DURATION {
                            should_recover = true;
                            recover_reason = "max_turn_exceeded".to_string();
                            error_msg = format!(
                                "Pane busy beyond maximum turn duration ({:?})",
                                super::MAX_TURN_DURATION
                            );
                        } else if duration_busy >= Duration::from_secs(60)
                            && pane.last_output_at <= *delivered_at
                        {
                            should_recover = true;
                            recover_reason = "no_output_after_delivery".to_string();
                            error_msg = "No output produced within expected startup/turn window"
                                .to_string();
                        }
                    }

                    // MCP helper hung check. The generated helper writes an
                    // in-flight marker before spawning `brehon serve`; if the
                    ...
                    if !should_recover && pane.is_agy() {
                        ...
                        {
                            should_recover = true;
                            recover_reason = "helper_call_hung".to_string();
                            error_msg = format!(
                    ...
                    // Terminal prompt is blocked on an approval/trust prompt
                    if !should_recover {
                        let is_blocked_on_trust = if let Ok(rows) = pane.viewport_rows_for_display()
                        {
                            rows.iter().any(|row| {
                        ...
                        if is_blocked_on_trust {
                            should_recover = true;
                            recover_reason = "blocked_on_approval".to_string();
                            error_msg = "Terminal prompt is blocked on an approval/trust prompt"
                                .to_string();
                        }
                    }
                }
            ...
            if should_recover {
                ...
                let consecutive_crashes = self
                    .panes
                    .get(&pane_id)
                    .map(|p| p.consecutive_crashes)
                    .unwrap_or(0);
                let counts_as_crash = Self::agy_recovery_reason_counts_as_crash(&recover_reason);
                if counts_as_crash && consecutive_crashes >= 2 {
                    // Quarantine the pane!
                    let death_reason = crate::pane::DeathReason::Quarantined(format!(
                        "Repeated crashes (consecutive_crashes={})",
                        consecutive_crashes
                    ));
                    self.quarantine(&pane_id, death_reason);
                    if let Some(pane) = self.panes.get(&pane_id) {
                        super::stability::write_agy_health_marker(
                            pane,
                            &recover_reason,
                            &error_msg,
                        );
                    ...
                    // Apply backoff check
                    let last_restart = self.panes.get(&pane_id).and_then(|p| p.last_restart_at);
    ...
    fn take_next_ready_queued_prompt(
        ...
        if let Some(in_flight) = pane.prompt_queue.in_flight.as_ref() {
            if in_flight.generation != pane.current_generation() {
