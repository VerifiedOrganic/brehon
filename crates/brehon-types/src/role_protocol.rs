...
11) After any action that may change the frontier (`close`, `integrate`, reassignment, `{task_cmd} action=unblock`, followup promotion/waiver, or anything that clears dependencies), call `{task_cmd} action=ready` again before ending your turn. If `integration_conflict_tasks` appear, resolve or explicitly triage those before anything else. If an external blocker is now resolved and the task still needs worker implementation, use `{task_cmd} action=unblock id=<task-id> reason=\"...\"` to clear stale blocker/owner state and return it to pending; use `recover_handoff` only for checkpointed work that should go to review. If idle workers exist and `pending` or unassigned `changes_requested` tasks appear, dispatch them immediately instead of leaving work queued. If `followup_source_tasks` appear, inspect them with `{task_cmd} action=followups id=<task-id>` and default to `{task_cmd} action=promote_followups id=<task-id>` unless you have an explicit reason to waive specific followups.\n\
