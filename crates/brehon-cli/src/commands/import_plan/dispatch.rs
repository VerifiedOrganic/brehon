...
fn phase_seam_steward_source_id(phase: &PlanPhase) -> String {
    format!("brehon.phase.{}.seam-steward", phase.id)
}

fn phase_source_task_ids(phase: &PlanPhase) -> Vec<String> {
    let mut ids = phase
        .epics
        .iter()
        .flat_map(|epic| epic.tasks.iter().map(|task| task.source_id.clone()))
        .collect::<Vec<_>>();
    if let Some(gate_task) = &phase.gate_task {
        ids.push(gate_task.source_id.clone());
    }
    ids
}

fn phase_seam_steward_task(phase: &PlanPhase) -> PlanTask {
    PlanTask {
        source_id: phase_seam_steward_source_id(phase),
        title: format!("Phase {} integration seam steward", phase.id),
        dependencies: phase_source_task_ids(phase),
        size: "S".to_string(),
        gate: format!(
            "Phase {} is integrated against the current initiative branch, no tracked .brehon artifacts exist, cross-phase API/config/test seams are either fixed or promoted as concrete follow-up tasks, and the phase branch passes the relevant integration gate.",
            phase.id
        ),
...
pub async fn execute(
    ...
    let initiative = call_task_tool(
        ...
        json!({
            ...
            "acceptance_criteria": plan.phases.iter().map(|phase| format!("Phase {} is closed after all imported source tasks and its phase gate are complete.", phase.id))
                .chain(std::iter::once("Each phase seam-steward task has checked the phase against the live initiative branch before that phase closes.".to_string()))
            ...
            "plan_steps": plan.phases.iter().map(|phase| format!("Phase {}: {}", phase.id, phase.title))
                .chain(std::iter::once("Run each synthetic phase seam-steward task before closing its phase epic.".to_string()))
                .chain(std::iter::once(format!("{FINAL_HARDENING_EPIC_TITLE}: seal final cross-epic gaps before initiative close.")))
                .collect::<Vec<_>>(),
            "implementation_notes": format!("Imported from {}. This initiative was generated by brehon import-plan and preserves the source phase/task DAG. Brehon adds phase seam-steward tasks so integration pressure happens throughout the run instead of only during final hardening.", plan.path.display()),
            "role": "supervisor",
    ...
    for phase in &plan.phases {
        ...
        phase_steps.push(format!(
            "Run the Brehon phase seam steward for Phase {} before closing the phase epic.",
            phase.id
        ));
        ...
        let epic = call_task_tool(
            ...
            json!({
                ...
                "implementation_notes": if phase.notes.is_empty() {
                    format!("Imported from {}. Source phase contains {} document epics. Brehon adds a phase seam-steward task that must merge into this branch after the source work and before the phase closes.", plan.path.display(), phase.epics.len())
                } else {
                    format!("{}\n\nImported from {}. Brehon adds a phase seam-steward task that must merge into this branch after the source work and before the phase closes.", phase.notes.join("\n"), plan.path.display())
                },
        ...
        let seam_task = phase_seam_steward_task(phase);
        let created = call_task_tool(
            &tool,
            json!({
                "action": "create",
                ...
                "parent_id": epic_id,
                "completion_mode": "merge",
                "title": seam_task.title,
                "description": format!("Synthetic Brehon integration steward for Phase {}. This task exists to catch cross-epic seams while the phase is still local to the initiative branch.", phase.id),
                "acceptance_criteria": [seam_task.gate.clone()],
                "file_hints": [
                    "Inspect the current phase integration branch and the parent initiative integration branch.",
                    "Look for API, config, test, docs, alarm/readiness, feature-flag, and crate-boundary drift introduced by this phase.",
                    "Check git status and tracked files so .brehon runtime artifacts cannot enter the product repository."
                ],
                "constraints": [
                    format!("Synthetic source task ID: {}", seam_task.source_id),
                    "Merge or rebase the current initiative branch into this phase branch before final checks if the parent branch advanced.",
                    "Fix small concrete seams directly in this phase branch; promote larger cleanup as explicit follow-up tasks before closing the phase.",
                    "Do not defer newly discovered cross-phase breakage to final hardening unless it is explicitly waived or represented as a concrete final-hardening task."
                ],
                "test_requirements": [seam_task.gate.clone()],
                "plan_steps": [
                    "Compare this phase branch against the current initiative integration branch.",
                    "Run the relevant workspace or cross-crate integration tests for the touched surface.",
                    "Verify no .brehon files are tracked or staged for merge.",
                    "Request review only after seam findings are fixed, promoted, or explicitly waived."
                ],
                "implementation_notes": "Generated by import-plan. This is continuous integration stewardship for the phase, not source-plan scope.",
                "role": "supervisor",
                "agent_name": "plan-importer"
            }),
        )
        ...
        imported_tasks.push(ImportedTaskRecord {
            brehon_task_id: created["task_id"]
                .as_str()
                .ok_or_else(|| anyhow!("Task tool did not return task_id for seam steward"))?
                .to_string(),
            phase_id: phase.id.clone(),
            phase_title: phase.title.clone(),
            source_epic_id: Some("brehon.integration".to_string()),
            source_epic_title: Some("Brehon integration stewardship".to_string()),
            task: seam_task,
            is_phase_gate: false,
        });
    }
