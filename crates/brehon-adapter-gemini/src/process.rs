...
pub struct AgentProcess {
    ...
    stdout_dropped_lines: Arc<AtomicU64>,
    stderr_dropped_lines: Arc<AtomicU64>,
...
impl AgentProcess {
    ...
    pub async fn spawn_with_env(
        ...
        let stdout_dropped_lines = Arc::new(AtomicU64::new(0));
        let stderr_dropped_lines = Arc::new(AtomicU64::new(0));
        ...
        let stdout_reader = Self::spawn_stdout_reader(
            ...
            Arc::clone(&stdout_dropped_lines),
        );
        let stderr_reader = Self::spawn_stderr_reader(
            process_id,
            BufReader::new(stderr),
            stderr_tx,
            Arc::clone(&stderr_dropped_lines),
        );
    ...
    pub fn stdout_dropped_lines(&self) -> u64 {
        self.stdout_dropped_lines.load(Ordering::SeqCst)
    }

    pub fn stderr_dropped_lines(&self) -> u64 {
        self.stderr_dropped_lines.load(Ordering::SeqCst)
    }
...
fn try_send_process_line(
    ...
    match tx.try_send(line) {
        ...
        Err(TrySendError::Full(_line)) => {
            let dropped = dropped_lines.fetch_add(1, Ordering::SeqCst) + 1;
            if dropped == 1 || dropped.is_power_of_two() {
                warn!(
                    process_id,
                    stream,
                    dropped,
                    "Agent process output channel full; dropping line to avoid stdout/stderr backpressure deadlock"
                );
            }
            true
        }
        Err(TrySendError::Closed(_line)) => false,
    }
}
